...
Since all coefficients are 1, the polynomial P(pi) is GP sum with a=1, d=pi and n=a+1
...
def d(n):
    ...
    for i in range(3, limit, 2):
        ...
        if count != 0:
            summation *= (i ** (count + 1) - 1) / (i - 1)

    if number > 2:
        summation *= (number ** 2 - 1) / (number - 1)
